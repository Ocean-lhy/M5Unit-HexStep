...
void lv_textprogress_set_value(lv_obj_t * obj, uint32_t value)
{
    ...
    if(tp->decimals > 0) {
        lv_snprintf(textbuffer, sizeof(textbuffer), "%d.%0*d%%", (int)(progress / range[tp->decimals]), tp->decimals,
                    (int)(progress % range[tp->decimals]));
    }
    else {
        lv_snprintf(textbuffer, sizeof(textbuffer), "%d%%", (int)progress);
    }
